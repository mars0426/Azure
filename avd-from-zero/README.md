# 從零開始建構 AVD 環境
本文透過實際演練，示範如何從零開始建構一整套 AVD (Azure Virtual Desktop) 環境，包含黃金映像的製作、混合式 AD 驗證、FSLogix 的設定等內容。
...
2. 建立 Virtual machine，Resource group 選擇「AVD-PoC」，Virtual machine name 輸入「Golden-Image」，Region 選擇「Japan East」，Image 選擇「Windows 10 Enterprise multi-session, version 22H2 + Microsoft 365 Apps - x64 Gen2」
  ...
  ![image](https://user-images.githubusercontent.com/42570850/212422175-a054627c-e624-4b79-b266-53f9759a58f5.png)
...
4. 安裝 FSLogix
  * Windows 10/11 Enterprise multi-session 不需要在安裝應用程式前執行 change user /install，或是在安裝後執行 change user /execute 指令
  * 參考 [Microsoft 文件](https://learn.microsoft.com/en-us/fslogix/install-ht)，下載 [FSLogix](https://aka.ms/fslogix-latest)
  * 解壓縮後執行 \x64\Release\FSLogixAppsSetup.exe
  * 勾選「I agree...」後點選「Install」

  ![image](https://user-images.githubusercontent.com/42570850/212428212-273cf245-92f1-4bc8-857a-bc82cbc54848.png)

5. 依實際需求，安裝 LOB (Line of Business) 應用程式及輸入法，例如 Acrobat Reader、Photoshop、AutoCAD 等
6. 執行 Windows update，確認無任何更新需要安裝

  ![image](https://user-images.githubusercontent.com/42570850/212209305-a019a1da-20aa-4aca-bf7c-06dacb7ce585.png)

7. 開啟 Command Prompt，執行下列 Sysprep 指令，執行完成後會自動關機
  ```
  %WINDIR%\system32\sysprep\sysprep.exe /generalize /shutdown /oobe /mode:vm
  ```
8. 到 Azure Portal，進入 Golden-Image 頁面，點選「Stop」，確認 Status 變成「Stopped (deallocated)」

  ![image](https://user-images.githubusercontent.com/42570850/212209576-bc1ebc9a-1e44-4f07-924a-74e6e2f4e4ac.png)

9. 點選「Capture」，Share image to Azure compute gallery 選擇「No, capture only a managed image.」，勾選「Automatically delete this virtual machine after creating the image」，Name 輸入「Golden-Image-AS」，點選「Review + create」→「Create」

  ![image](https://user-images.githubusercontent.com/42570850/212210347-2190b240-389c-4ef1-b117-026f4d367bd4.png)

10. 將資源群組 AVD-PoC 中除了 Golden-Image-AS 之外的其他資源刪除，只保留 Golden-Image-AS
